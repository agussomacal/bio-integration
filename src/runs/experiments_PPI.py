...
global_params["analysisname"] = "Experiments_PPI"
...
for integrator_name, integrator in integrators_dict.items():
    ...
    test_name = evaluator.metric_name + "_" + integrator_name
    ...
    filename = Wpath + "/{}_PPI_BP_integrated".format(test_name)
    tpe_results.to_csv(filename+".csv")

    visualizators.plot_optimization(x_variable="PPI",
                                    y_variable=evaluator.metric_name,
                                    optimizer=optimizer,
                                    tpe_results=tpe_results,
                                    color="purple",
                                    filename=filename,
                                    baseline={"value": (auc_ppi, auc_bp),
                                              "color": (ppi_params["color"], bp_params["color"]),
                                              "label": ("ppi alone", "bp alone")})
...
tpe_results.to_csv(filename+".csv")
visualizators.plot_optimization(x_variable="lambda exponent",
                                y_variable=evaluator.metric_name,
                                optimizer=optimizer,
                                tpe_results=tpe_results,
                                color="orange",
                                filename=filename,
                                baseline={"value": auc_ppi,
                                          "color": ppi_params["color"],
                                          "label": "baseline: exponent -0.5"})
...
tpe_results.to_csv(filename+".csv")
visualizators.plot_optimization(x_variable="alpha",
                                y_variable=evaluator.metric_name,
                                optimizer=optimizer,
                                tpe_results=tpe_results,
                                color="green",
